...
(function () {
  ...
  var COURSE_STEPS = [
    { id: 'step1', label: 'שלב 1: הצג את הבעיה בצורה סטנדרטית וקבע פתרון בסיסי התחלתי (B, B⁻¹)' },
    { id: 'step2', label: 'שלב 2: הערך את הפתרון הבסיסי — חשב את xB ואת Z' },
    { id: 'step3', label: 'שלב 3: מי נכנס? — חשב את yᵀ ואת rN' },
    { id: 'step4', label: 'שלב 4: מי יוצא? — חשב את n̄q ובצע מבחן יחס' },
    { id: 'step5', label: 'שלב 5: עדכן את B ואת B⁻¹ וחזור לשלב 2' },
  ];
  ...
  var Q_HINTS = {
    ...
    xB: {
      what: 'שלב 2: ערכי המשתנים הבסיסיים בפתרון הנוכחי.',
    ...
    y: {
      what: 'שלב 3: הווקטור yᵀ (באיטרציה האחרונה — מחירי הצל!).',
      how: 'yᵀ = cB · B⁻¹.',
      dims: 'וקטור שורה: 1×m.',
    },
    rN: {
    ...
    nBarQ: {
      what: 'שלב 4: עמודת המשתנה הנכנס כפי שהיא נראית בטבלו הנוכחי.',
  ...
  var STEP_WHY = {
    ...
    step5: 'יש נכנס ויש יוצא — נותר לעדכן את B, B⁻¹ והנגזרות שלהם, ולחזור להערכת הפתרון (שלב 2).',
  };
  ...
  function describeSession(s) {
    ...
    if (s.phase === 'setup') {
      lines.push('מצב: הקמה — קביעת הבסיס ההתחלתי (שלב 1).');
